...
public final class P2pMgr implements IP2pMgr {
					...
					try {
						...
						if (channel.finishConnect() && channel.isConnected()) {
							...
							workers.submit(new TaskWrite(workers, showLog, node.getIdShort(), channel,
									cachedReqHandshake1, rb));
							workers.submit(new TaskWrite(workers, showLog, node.getIdShort(), channel,
									cachedReqHandshake, rb));
			...
			if (handshakeRuleCheck(_netId)) {
				...
				if (_revision != null) {
					...
					workers.submit(new TaskWrite(workers, showLog, node.getIdShort(), node.getChannel(),
							cachedResHandshake1, _buffer));
				}
				// handshake 0
				else {
					workers.submit(new TaskWrite(workers, showLog, node.getIdShort(), node.getChannel(),
							cachedResHandshake, _buffer));
				}
	...
	public void run() {
		try {
			...
			workers = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() * 2, new ThreadFactory() {
			...
			scheduledWorkers.scheduleWithFixedDelay(new TaskPersistNodes(nodeMgr), 30000, 10, TimeUnit.SECONDS);
	...
	public void send(int _nodeIdHashcode, final Msg _msg) {
		...
		if (node != null) {
			...
			if (sk != null) {
				...
				if (attachment != null)
					workers.submit(new TaskWrite(workers, showLog, node.getIdShort(), node.getChannel(), _msg,
