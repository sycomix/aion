...
public final class P2pMgr implements IP2pMgr {
				...
				if (!nodeMgr.getOutboundNodes().containsKey(nodeIdHash) && !nodeMgr.hasActiveNode(nodeIdHash)) {
					...
					try {
						...
						if (channel.finishConnect() && channel.isConnected()) {
							...
							// if don't sleep a while and direct send handshake, sometime it just failed by without response.
							try {
								Thread.sleep(1000);
							} catch (Exception e) {
							}
	...
	public void run() {
		try {
			...
			Thread thrdSend = new Thread(new TaskSend(), "p2p-send");
			thrdSend.setPriority(Thread.NORM_PRIORITY);
			thrdSend.start();
