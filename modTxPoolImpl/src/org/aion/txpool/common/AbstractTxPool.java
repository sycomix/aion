	...
	protected void sortTxn() {
		...
		this.mainMap.entrySet().parallelStream().forEach(e -> {
			...
			synchronized (accMap) {
				...
				// try {
				// if
				// (this.mainMap.get(nonceMap.get(nonce).getKey()).getTx().getTimeStampBI().compareTo(tx.getTimeStampBI())
				// < 1) {
					...
					if (txNonceStart != null) {
						...
						for (PoolState ps : psl) {
							...
							if (isClean(ps, as) && ps.firstNonce.equals(txNonceStart)
								...
								Map<ByteArrayWrapper, TxDependList<ByteArrayWrapper>> txDp = this.feeView
										.get(ps.getFee());
								if (txDp != null) {
