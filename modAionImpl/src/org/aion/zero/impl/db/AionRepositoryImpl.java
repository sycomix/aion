...
public class AionRepositoryImpl extends AbstractRepository<AionBlock, A0BlockHeader, AionBlockStore> {
	 ...
	 *
	 * @ATTENTION: when do snap shot, another instance will be created. Make sure it
	 *             is used only by getSnapShotTo
	 */
	...
	private static class AionRepositoryImplHolder {
		...
		private final static AionRepositoryImpl inst = new AionRepositoryImpl(new RepositoryConfig(
				new String[] { config.getDb().getVendor() }, config.getDb().getVendor(),
				...
				config.getDb().getBlockSize(), config.getDb().getWriteBufferSize(), config.getDb().getCacheSize()));
	}
	...
	private void init() {
		try {
			...
			this.transactionStore = new TransactionStore<>(txDB, AionTransactionStoreSerializer.serializer);
	...
	private Trie createStateTrie() {
		return new SecureTrie(stateDB).withPruningEnabled(pruneBlockCount >= 0);
	}
	...
	/**
	 * @implNote The loaded objects are fresh copies of the original account state
	...
	public synchronized void commitBlock(A0BlockHeader blockHeader) {
		...
		/*
		 * if (pruneBlockCount >= 0) { stateDSPrune.storeBlockChanges(blockHeader);
	...
	// temporarily removed since never used
	/*
	 * private void pruneBlocks(A0BlockHeader curBlock) { if (curBlock.getNumber() >
	 * bestBlockNumber) { // pruning only on // increasing blocks long
	 * pruneBlockNumber = curBlock.getNumber() - pruneBlockCount; if
	 * (pruneBlockNumber >= 0) { byte[] pruneBlockHash =
	 * blockStore.getBlockHashByNumber(pruneBlockNumber); if (pruneBlockHash !=
	 * null) { A0BlockHeader header =
	 * blockStore.getBlockByHash(pruneBlockHash).getHeader(); //
	 * stateDSPrune.prune(header); // detailsDS.getStorageDSPrune().prune(header); }
	...
	/**
	 * This function cannot for any reason fail, otherwise we may have dangling file
	 * IO locks
	 */
	@Override
	public void close() {
	...
	/**
	 * Retrieves the underlying state database that sits below all caches. This is
	 * usually provided by {@link org.aion.db.impl.leveldb.LevelDB} or
	 * {@link org.aion.db.impl.leveldb.LevelDB}.
	 * <p>
	 * Note that referencing the state database directly is unsafe, and should only
	 * be used for debugging and testing purposes.
	 *
	 * @return
	 */
	...
	/**
	 * Retrieves the underlying details database that sits below all caches. This is
	 * usually provided by {@link org.aion.db.impl.mockdb.MockDB} or
	 * {@link org.aion.db.impl.mockdb.MockDB}.
	 * <p>
	 * Note that referencing the state database directly is unsafe, and should only
	 * be used for debugging and testing purposes.
	 *
