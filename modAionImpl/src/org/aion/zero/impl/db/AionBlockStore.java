...
public class AionBlockStore extends AbstractPowBlockstore<AionBlock, A0BlockHeader> {
	...
	// index db
	private IBytesKVDB indexDS;

	private DataSourceArray<List<BlockInfo>> index;

	// block db
	private IBytesKVDB blocksDS;

	private ObjectDS<AionBlock> aionBlocks;

	public AionBlockStore(IBytesKVDB index, IBytesKVDB blocks) {
		init(index, blocks);
	}

	private void init(IBytesKVDB index, IBytesKVDB blocks) {

		indexDS = index;
		this.index = new DataSourceArray<>(new ObjectDS<>(index, BLOCK_INFO_SERIALIZER));
		this.blocksDS = blocks;

		this.aionBlocks = new ObjectDS<>(blocks, new Serializer<AionBlock, byte[]>() {
			@Override
	...
	// @TODO:
	// bad implementation. tuple can be more efficient vs map->entry->entry.
	public List<Map.Entry<AionBlock, Map.Entry<BigInteger, Boolean>>> getBlocksByNumber(long number) {
		...
		for (BlockInfo blockInfo : blockInfos) {
			...
			AionBlock block = aionBlocks.get(hash);
	...
	public boolean isBlockExist(byte[] hash) {
		return aionBlocks.get(hash) != null;
	}
	...
	public List<AionBlock> getListBlocksEndWith(byte[] hash, long qty) {
		...
		AionBlock block = this.aionBlocks.get(hash);
	...
	private void loopBackToCommonBlock(IAionBlock bestLine, IAionBlock forkLine) {
		...
		AionLoggerFactory.getLogger(LogEnum.CONS.name()).info("branching: common block = {}/{}", forkLine.getNumber(),
	...
	public void revert(long previousLevel) {
		...
		while (currentLevel > previousLevel) {
			...
			if (currentLevelBlocks == null || currentLevelBlocks.size() == 0) {
				aionBlocks.delete(bestLine.getHash());
				LOG.error("Null block information found at " + currentLevel + " when information should exist.");
			} else {
				for (BlockInfo bk_info : currentLevelBlocks) {
					aionBlocks.delete(bk_info.getHash());
				}
			}
	...
	private void pruneSideChains(IAionBlock block) {
		...
		for (BlockInfo wrongBlock : levelBlocks) {
			aionBlocks.delete(wrongBlock.getHash());
		}
	...
	public static class BlockInfo implements Serializable {
		...
		public boolean decode(byte[] ser) {
		...
		// not easy link blockinfo to parent as it's level based.
		// BlockInfo parent;
	...
	/**
	 * Called by {@link AionBlockStore#BLOCK_INFO_SERIALIZER} for now, on main-net
	 * launch we should default to this class.
	 */
	public static final Serializer<List<BlockInfo>, byte[]> BLOCK_INFO_RLP_SERIALIZER = new Serializer<>() {
		...
		public List<BlockInfo> deserialize(byte[] stream) {
			...
			for (RLPElement aList : list) {
				...
				boolean ret = bi.decode(aList.getRLPData());
				if (ret)
					res.add(bi);
			}
			return res;
